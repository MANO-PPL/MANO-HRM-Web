...
import { formatDateSafe, getMonthBounds, buildWageRateResolver } from './labourService.js';

/**
 * Service to generate the Complete 3-Row Daily Spreadsheet & Monthly Wage Ledger Excel workbook
 * Balance Payable = Gross Amount (Base + OT) - Total Advances (independent of recorded payouts)
 *
 * @param {Object} params
 * @param {number} params.org_id
 * @param {string|number} params.site_id
 * @param {string} [params.month]
 * @returns {Promise<{ workbook: ExcelJS.Workbook, filename: string }>}
 */
export async function generateDetailedMonthlyLedgerWorkbook({ org_id, site_id, month }) {
    if (!site_id) {
