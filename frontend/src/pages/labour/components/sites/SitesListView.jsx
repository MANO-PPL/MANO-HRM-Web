    ...
    return (
        <div className="space-y-3 animate-in fade-in duration-200">
            ...
            <div className="bg-white dark:bg-[#0d1117] border border-slate-200 dark:border-[#30363d] rounded-xl shadow-xs overflow-hidden">
                {sites.length === 0 ? (
                    <div className="border border-dashed border-slate-300 dark:border-[#30363d] rounded-xl p-10 text-center m-4">
                        <Building className="mx-auto text-slate-300 dark:text-[#8b949e] mb-2" size={32} />
                        <h4 className="text-xs font-medium text-slate-500 dark:text-[#8b949e]">No Construction Sites Found</h4>
                        <p className="text-[10px] text-slate-400 dark:text-[#8b949e] mt-1">Create a site first to start assigning labour forces.</p>
                    </div>
                ) : (
                                ...
                                .map(site => {
                                    ...
                                    return (
                                        <tr
                                            ...
                                            className="border-b border-slate-100 dark:border-[#21262d] hover:bg-indigo-50/40 dark:hover:bg-[#161b22]/70 cursor-pointer transition-colors group"
                                        >
                                            <td className="p-3">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className="font-semibold text-slate-800 dark:text-[#f0f6fc] group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
                                                        {site.site_name}
                                                    </span>
                                                </div>
                                            </td>
                                            <td className="p-3 text-slate-500 dark:text-[#8b949e] max-w-[200px]">
                                                <span className="line-clamp-1">{site.location_details || <span className="italic text-slate-400 dark:text-[#8b949e]">No details</span>}</span>
                                            </td>
                                            <td className="p-3 text-center">
                                                <span className={`inline-flex px-2 py-0.5 text-[10px] font-medium rounded-full uppercase ${site.status === 'Active'
                                                        ? 'bg-emerald-50 text-emerald-600 dark:bg-emerald-950/20 dark:text-emerald-400'
                                                        : site.status === 'Completed'
                                                            ? 'bg-blue-50 text-blue-600 dark:bg-blue-950/20 dark:text-blue-400'
                                                            : 'bg-slate-100 text-slate-500 dark:bg-[#21262d] dark:text-[#c9d1d9]'
                                                    }`}>
                                                    {site.status}
                                                </span>
                                            </td>
                                            <td className="p-3 text-center">
                                                <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-indigo-50 dark:bg-indigo-950/20 text-indigo-600 dark:text-indigo-400 font-medium text-[11px]">
                                                    <Users size={11} />
                                                    {assignedCount}
                                                </span>
                                            </td>
                                            ...
                                            <td className="p-3 text-right" onClick={(e) => e.stopPropagation()}>
                                                <div className="flex justify-end gap-1.5">
                                                    ...
                                                    <button
                                                        ...
                                                        className="p-1.5 hover:bg-red-50 dark:hover:bg-red-950/20 text-red-500 rounded border border-slate-200 dark:border-[#30363d] transition-colors"
                                                        title="Delete site"
                                                    >
                                                        <Trash2 size={12} />
                                                    </button>
