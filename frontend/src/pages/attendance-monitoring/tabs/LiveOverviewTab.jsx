...
const extractShortLocation = (address) => {
    if (!address || typeof address !== 'string') return null;
    const trimmed = address.trim();
    if (['Manual Correction', 'Checkpoint', 'Unknown', 'System Checkout', 'Locating...'].includes(trimmed)) {
        return trimmed;
    }
    // If coordinates format e.g. "19.02, 72.85"
    if (/^-?\d+\.\d+,\s*-?\d+\.\d+$/.test(trimmed)) {
        const parts = trimmed.split(',').map(n => Number(n).toFixed(2));
    ...
    if (meaningful.length > 3) {
        ...
        if (['india', 'usa', 'united states', 'uk', 'uae', 'congo'].some(c => last.includes(c))) {
            meaningful.pop();
        }
    }
    // Remove state / postal code if present (e.g. 'Maharashtra 400019' or 'Maharashtra')
    if (meaningful.length > 2) {
        const last = meaningful[meaningful.length - 1];
        if (/\d{5,6}/.test(last) || /maharashtra|karnataka|delhi|gujarat|tamil nadu/i.test(last)) {
            meaningful.pop();
        }
    }
    const city = meaningful[meaningful.length - 1];
    const area = meaningful.length >= 2 ? meaningful[meaningful.length - 2] : null;

    if (area && city && area.toLowerCase() !== city.toLowerCase()) {
        const cleanArea = area.replace(/^[A-Z0-9\+]+\s+/, '').trim();
...
const LiveOverviewTab = ({
    ...
    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 gap-4">
            {loading ? (
                <div className="col-span-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 gap-4 animate-pulse">
                    {Array.from({ length: 10 }).map((_, i) => (
                        <div key={i} className="bg-white dark:bg-dark-card rounded-xl border border-slate-200 dark:border-github-dark-border/60 p-4 space-y-3">
                            <div className="flex items-center gap-3">
                                <div className="w-11 h-11 rounded-lg bg-slate-200 dark:bg-slate-700/60"></div>
                                <div className="space-y-1.5 flex-1">
                                    <div className="h-3.5 bg-slate-200 dark:bg-slate-700/60 rounded w-3/4"></div>
                                    <div className="h-2.5 bg-slate-150 dark:bg-slate-800/80 rounded w-1/2"></div>
                                </div>
                            </div>
                            <div className="h-5 bg-slate-100 dark:bg-slate-800/80 rounded w-1/3"></div>
                            <div className="grid grid-cols-2 gap-2 pt-1">
                                <div className="h-8 bg-slate-100 dark:bg-slate-800/50 rounded-lg"></div>
                                <div className="h-8 bg-slate-100 dark:bg-slate-800/50 rounded-lg"></div>
                            </div>
                        </div>
                    ))}
                ...
                filteredData.map((item, index) => {
                    ...
                    return (
                        <React.Fragment key={item.id}>
                            {showDivider && (
                                <div className="col-span-full py-5 flex items-center gap-4">
                            ...
                            >
                                ...
                                <div className="p-4 flex items-start justify-between">
                                    <div className="flex gap-3 min-w-0">
                                        <div className={`w-11 h-11 shrink-0 rounded-lg flex items-center justify-center font-semibold text-base shadow-sm overflow-hidden ${item.status === 'Absent' ? 'bg-slate-100 text-slate-400 dark:bg-github-dark-subtle dark:text-github-dark-muted' : 'bg-gradient-to-br from-indigo-500 to-purple-600 text-white'}`}>
                                        ...
                                        <div className="min-w-0 flex-1">
                                            <h3 className="font-semibold text-slate-800 dark:text-github-dark-text text-sm truncate" title={item.name}>{item.name}</h3>
                                            <p className="text-xs text-slate-500 dark:text-github-dark-muted font-normal mt-0.5 truncate">{item.role}</p>
                                        </div>
                                    </div>
                                    <button className="text-slate-300 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors shrink-0 ml-1">
                                ...
                                <div className="px-4 pb-3 flex flex-wrap items-center gap-1.5">
                                    {item.allStatuses && item.allStatuses.map(statusBadge => (
                                        <span key={statusBadge} className={`inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-medium border shadow-xs ${resolveStatusStyle(statusBadge).replace('bg-', 'bg-opacity-10 border-').replace('text-', 'text-')}`}>
                                ...
                                {/* Divider */}
                                <div className="h-px bg-slate-100 dark:bg-github-dark-subtle mx-4"></div>

                                {/* Card Body - Latest Session Only */}
                                <div className="p-4 flex-1 overflow-hidden">
                                    {item.status === 'Absent' ? (
                                        <div className="h-full flex flex-col items-center justify-center text-slate-400 py-3 italic font-normal">
                                            <Clock size={18} className="mb-1.5 opacity-30" />
                                            <span className="text-xs">No activity yet</span>
                                        </div>
                                    ) : item.status === 'Leave' ? (
                                        <div className="h-full flex flex-col items-center justify-center text-purple-600 dark:text-purple-400 py-3 font-normal">
                                            <div className="w-7 h-7 rounded-full bg-purple-50 dark:bg-purple-950/30 flex items-center justify-center mb-1">
                                        ...
                                        <div className="h-full flex flex-col items-center justify-center text-slate-400 py-3 font-normal">
                                            <span className="text-xs font-medium">Scheduled Week Off</span>
                                        </div>
                                    ) : item.status === 'Holiday' ? (
                                        <div className="h-full flex flex-col items-center justify-center text-sky-500 py-3 font-normal">
                                        ...
                                        <div className="relative pl-3.5 border-l-2 border-indigo-500">
                                            ...
                                            <div className="flex items-center justify-between mb-1.5">
                                                <span className="text-[11px] font-medium text-slate-500 dark:text-github-dark-muted">
                                                    Latest Session
                                                </span>
                                            </div>
                                            ...
                                            <div className="grid grid-cols-2 gap-2">
                                                <div className="space-y-0.5 min-w-0">
                                                    <div className="flex items-center gap-1.5 text-xs text-slate-600 dark:text-slate-300 font-normal">
                                                        ...
                                                        <span className="truncate">In <span className="font-semibold text-slate-800 dark:text-slate-100">{item.sessions[0].in}</span></span>
                                                    </div>
                                                    {extractShortLocation(item.sessions[0].inLocation) && (
                                                        <div className="flex items-center gap-1 text-[10px] text-slate-400 dark:text-slate-500 pl-3 min-w-0" title={item.sessions[0].inLocation}>
                                                            <MapPin size={9} className="shrink-0 text-indigo-400" />
                                                            <span className="truncate">{extractShortLocation(item.sessions[0].inLocation)}</span>
                                                        </div>
                                                    )}
                                                </div>
                                                ...
                                                <div className="space-y-0.5 min-w-0">
                                                    <div className="flex items-center gap-1.5 text-xs text-slate-600 dark:text-slate-300 font-normal">
                                                        <div className={`w-1.5 h-1.5 rounded-full shrink-0 ${item.sessions[0].isActive ? 'bg-slate-300 dark:bg-slate-600' : 'bg-red-500'}`}></div>
                                                        <span className="truncate">Out <span className="font-semibold text-slate-800 dark:text-slate-100">{item.sessions[0].isActive ? 'Ongoing...' : item.sessions[0].out}</span></span>
                                                    </div>
                                                    {!item.sessions[0].isActive && extractShortLocation(item.sessions[0].outLocation) && (
                                                        <div className="flex items-center gap-1 text-[10px] text-slate-400 dark:text-slate-500 pl-3 min-w-0" title={item.sessions[0].outLocation}>
                                                            <MapPin size={9} className="shrink-0 text-rose-400" />
                                                            <span className="truncate">{extractShortLocation(item.sessions[0].outLocation)}</span>
                                                        </div>
                                                    )}
                                                </div>
                                            </div>
                                            ...
                                            {item.sessions.length > 1 && (
                                                <div className="mt-2.5 text-center">
                                                    <span className="text-[10px] text-indigo-600 dark:text-indigo-400 font-medium bg-indigo-50 dark:bg-indigo-900/30 px-2 py-0.5 rounded-full cursor-pointer hover:bg-indigo-100 transition-colors">
                                ...
                                {item.status !== 'Absent' && item.status !== 'Leave' && item.status !== 'Week Off' && item.status !== 'Holiday' && (
                                    <div className="bg-slate-50 dark:bg-github-dark-subtle/50 px-4 py-2.5 border-t border-slate-100 dark:border-github-dark-border flex items-center justify-between">
