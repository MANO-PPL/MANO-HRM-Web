    ...
    return (
        ...
        >
            ...
            <div className="flex items-start justify-between gap-2 border-b border-slate-800 dark:border-[#30363d] pb-2">
                <div>
                    <h4 className="font-semibold text-xs text-slate-100 dark:text-[#f0f6fc] leading-tight">
                        {hoveredRecord.user_name}
                    </h4>
                    <p className="text-[9px] text-slate-400 dark:text-[#8b949e] mt-0.5 font-normal">
                        {hoveredRecord.date}
                    </p>
                </div>
            ...
            {hoveredRecord.status === 'Absent' ? (
                <p className="text-[10px] text-rose-400 font-normal italic">No attendance recorded. Marked absent.</p>
            ) : hoveredRecord.status === 'On Leave' ? (
                <p className="text-[10px] text-sky-400 font-normal italic">Approved leave for this day.</p>
            ) : (
                <div className="space-y-2">
                    {/* Punch Times */}
                    <div className="grid grid-cols-2 gap-2 text-[10px]">
                        <div>
                            <span className="text-slate-400 dark:text-[#8b949e] block font-normal">Punch In</span>
                            <span className="font-semibold text-slate-200 dark:text-[#c9d1d9]">{hoveredRecord.time_in || 'N/A'}</span>
                        </div>
                        <div>
                            <span className="text-slate-400 dark:text-[#8b949e] block font-normal">Punch Out</span>
                            <span className="font-semibold text-slate-200 dark:text-[#c9d1d9]">{hoveredRecord.time_out || (hoveredRecord.is_active ? 'In Progress' : 'N/A')}</span>
                        </div>
                    </div>
                                ...
                                <div key={i} className="relative h-14 rounded-lg border border-slate-700 dark:border-[#30363d] overflow-hidden bg-slate-900 shadow-sm flex flex-col justify-between">
                                    {item.img ? (
                                        ...
                                        <div className="flex flex-col items-center justify-center h-full pb-2 gap-0.5 opacity-40">
                                            ...
                                            <span className="text-[5px] font-medium uppercase tracking-wider text-slate-400">No Photo</span>
                                        </div>
                                    )}
                                    <div className="absolute bottom-0 left-0 right-0 bg-black/75 text-[6px] text-center uppercase py-0.5 font-medium tracking-wider text-white leading-none border-t border-slate-800/40">
                                        Punch {item.label}
                                    </div>
                                </div>
                            ))}
